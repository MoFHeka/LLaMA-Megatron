...
class Encoder(object):
    ...
    def split_json(self, json_line):
        data = json.loads(json_line)
        output = self._split_imp(data)
        return json.dumps(output), len(json_line)

    def split_txt(self, text_line):
        data = {"text": text_line}
        output = self._split_imp(data)
        return output, len(text_line)

    def _split_imp(self, data):
        output = {}
    ...
    def encode_json(self, json_line):
        data = json.loads(json_line)
        ids, lens = self._encode_imp(data)
        return ids, lens, len(json_line)

    def encode_txt(self, text_line):
        data = {"text": text_line}
        ids, lens = self._encode_imp(data)
        return ids, lens, len(text_line)

    def _encode_imp(self, data):
        ids = {}
        lens = {}
        for key in self.args.json_keys:
            text = data[key]
...
class Partition(object):
    ...
    def split_sentences(self, file_name):
        ...
        if "json" in os.path.splitext(input_file_name)[-1]:
            split_docs = pool.imap(encoder.split_json, fin, 32)
        elif "txt" in os.path.splitext(input_file_name)[-1]:
            split_docs = pool.imap(encoder.split_txt, fin, 32)
        else:
    ...
    def process_json_file(self, file_name):
        ...
        pool = multiprocessing.Pool(self.workers, initializer=encoder.initializer)
        if "json" in os.path.splitext(input_file_name)[-1]:
            encoded_docs = pool.imap(encoder.encode_json, fin, 32)
        elif "txt" in os.path.splitext(input_file_name)[-1]:
            encoded_docs = pool.imap(encoder.encode_txt, fin, 32)
        else:
            raise NotImplemented()
...
def get_args():
    ...
    group.add_argument(
        "--vocab-size", default=786, help="size of vocab for use with NullTokenizer"
    )
    group.add_argument(
        "--merge-file",
        type=str,
