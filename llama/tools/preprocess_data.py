...
def main():
    ...
    if 'json' in os.path.splitext(args.input)[-1]:
        encoded_docs = pool.imap(encoder.encode_json, fin, args.chunk_size)
        #encoded_docs = map(encoder.encode_json, fin)
    elif 'txt' in os.path.splitext(args.input)[-1]:
        encoded_docs = pool.imap(encoder.encode_txt, fin, args.chunk_size)
        #encoded_docs = map(encoder.encode_txt, fin)
    else:
