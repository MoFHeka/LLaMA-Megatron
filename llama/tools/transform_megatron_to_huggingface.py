...
def prepare_v2():
    ...
    id2info['transformers'] = {'layer' + str(idx): {str(tp_index): {} for tp_index in range(TP_SIZE)} for idx in
                               range(LAYER_NUM)}
    ...
    for tp_index in range(TP_SIZE):
        for pp_index in range(PP_SIZE):
            ...
            for k, v in encoder.items():
                if 'final_layernorm' in k: continue
...
def transform_meg_to_hug(meg_state_info, hug_state_dict):
    ...
    for hug_key, hug_para in hug_state_dict.items():
        if "inv_freq" in hug_key: continue
        if "embed_tokens" in hug_key:
            ...
            hug_state_dict[hug_key] = meg_emb_concat
            '''
            ...
            hug_state_dict[hug_key] = meg_final_layernorm
    ...
    for layer_index in range(LAYER_NUM):
        ...
        meg_qkvs = torch.concat(
            [meg_paras[str(tp_index)]['layers.' + str(meg_layer_index) + '.self_attention.query_key_value.weight'] for
             tp_index in range(TP_SIZE)], 0)
        HEAD_DIM = int(meg_qkvs.size()[0] // (HEAD_NUM * 3))

        meg_qkvs_3d = meg_qkvs.view(HEAD_NUM, 3, HEAD_DIM, HIDDEN_DIM)

        q, k, v = torch.chunk(meg_qkvs_3d, 3, 1)
        ...
        hug_state_dict[hug_layer_name_prefix + '.self_attn.q_proj.weight'] = q
        hug_state_dict[hug_layer_name_prefix + '.self_attn.k_proj.weight'] = k
        hug_state_dict[hug_layer_name_prefix + '.self_attn.v_proj.weight'] = v

        # recover gate_up

        meg_gate_up = torch.concat(
            [meg_paras[str(tp_index)]['layers.' + str(meg_layer_index) + '.mlp.dense_h_to_4h.weight'] for tp_index in
             range(TP_SIZE)], 0)
        ...
        hug_state_dict[hug_layer_name_prefix + '.mlp.gate_proj.weight'] = gate
        hug_state_dict[hug_layer_name_prefix + '.mlp.up_proj.weight'] = up
        ...
        hug_state_dict[hug_layer_name_prefix + '.input_layernorm.weight'] = meg_input_layernorm
        hug_state_dict[hug_layer_name_prefix + '.post_attention_layernorm.weight'] = meg_post_attention_layernorm

        # recover down

        meg_down = torch.concat(
            [meg_paras[str(tp_index)]['layers.' + str(meg_layer_index) + '.mlp.dense_4h_to_h.weight'] for tp_index in
             range(TP_SIZE)], 1)
        hug_state_dict[hug_layer_name_prefix + '.mlp.down_proj.weight'] = meg_down

        # recover output_proj
        meg_output = torch.concat(
            [meg_paras[str(tp_index)]['layers.' + str(meg_layer_index) + '.self_attention.dense.weight'] for tp_index in
             range(TP_SIZE)], 1)
        hug_state_dict[hug_layer_name_prefix + '.self_attn.o_proj.weight'] = meg_output
